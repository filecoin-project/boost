...
func TestE2E(t *testing.T) {
	...
	req.NoError(err)
	bifrostReady := testcmd.NewStderrWatcher("Path gateway listening on ")
	...
	cmdBifrost := tr.Start(ctx, testcmd.Args(bifrostGateway, "--gateway-port", fmt.Sprintf("%d", bifrostPort), "--metrics-port", fmt.Sprintf("%d", bifrostMetricsPort)), bifrostReady)
