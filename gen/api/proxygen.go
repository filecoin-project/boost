...
func generate(path, pkg, outpkg, outfile string) error {
	...
	if len(pkgs) == 0 {
		return fmt.Errorf("no packages found in %s", apiDir)
	}
	...
	if ap == nil {
		return fmt.Errorf("package %s not found in %s", pkg, apiDir)
	}
	if len(ap.Errors) > 0 {
		return ap.Errors[0]
	}
	...
	for i, fn := range ap.CompiledGoFiles {
		if i >= len(ap.Syntax) {
			continue
		}
		f := ap.Syntax[i]
	...
	err = doTemplate(w, m, `// Code generated by github.com/filecoin-project/boost/gen/api. DO NOT EDIT.
