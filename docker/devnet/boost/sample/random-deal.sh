...
###################################################################################
printf "${ci}boost deal --verified=false --provider=$PROVIDER \
...
boost deal --verified=false --provider=$PROVIDER --http-url=http://demo-http-server/$PAYLOAD_CID.car --commp=$COMMP_CID --car-size=$CAR --piece-size=$PIECE --payload-cid=$PAYLOAD_CID --storage-price 20000000000
