...
//	testData := shared_tut.NewLibp2pTestData(ctx, t)
//
...
//	pieceCid := shared_tut.GenerateCids(1)[0]
...
//		},
//		AskStore: modules.NewRetrievalAskGetter(),
//	}
//
...
//	retrievalPeer := legacyretrievaltypes.RetrievalPeer{
...
//	shared_tut.StartAndWaitForReady(ctx, t, client)
//
//	if tc.watch != nil {
...
//	client.SubscribeToEvents(func(event legacyretrievaltypes.ClientEvent, state legacyretrievaltypes.ClientDealState) {
//		tlog.Debugf("clnt mkt: %s %s %s", event.String(), state.Status.String(), state.Message)
//		switch event {
//		case legacyretrievaltypes.ClientEventComplete:
//			clientResChan <- nil
//		case legacyretrievaltypes.ClientEventCancelComplete:
//			clientResChan <- clientCancelled
//		case legacyretrievaltypes.ClientEventDealRejected:
//			clientResChan <- fmt.Errorf("%s :%w", state.Message, clientRejected)
//		case legacyretrievaltypes.ClientEventDataTransferError:
...
//	)).Node()
//	params, err := legacyretrievaltypes.NewParamsV1(abi.NewTokenAmount(0), 0, 0, sel, nil, abi.NewTokenAmount(0))
//	require.NoError(t, err)
...
//}
//
//func createRetrievalClient(ctx context.Context, t *testing.T, testData *shared_tut.Libp2pTestData, retrievalClientNode *testnodes.TestRetrievalClientNode) legacyretrievaltypes.RetrievalClient {
